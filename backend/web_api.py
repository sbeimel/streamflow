...
def stop_all_services():
    """Stop all running services (Automation + Stream Checker).
    
    This is an emergency stop that halts all background processing:
    - Stops automation service
    - Stops stream checker service
    - Clears stream checker queue
    ...
    try:
        results = {
            'automation': {'stopped': False, 'error': None},
            'stream_checker': {'stopped': False, 'error': None, 'queue_cleared': False}
        }
        ...
        try:
            manager = get_automation_manager()
            manager.stop_automation()
            results['automation']['stopped'] = True
            logger.info("✓ Automation service stopped")
        except Exception as e:
            results['automation']['error'] = str(e)
            logger.error(f"Failed to stop automation: {e}")
        
        # Stop Stream Checker Service
        try:
            service = get_stream_checker_service()
            service.stop()
            results['stream_checker']['stopped'] = True
            logger.info("✓ Stream checker service stopped")
            
            # Clear queue
            try:
                service.check_queue.clear()
                results['stream_checker']['queue_cleared'] = True
                logger.info("✓ Stream checker queue cleared")
            except Exception as e:
                logger.warning(f"Failed to clear queue: {e}")
        except Exception as e:
            results['stream_checker']['error'] = str(e)
            logger.error(f"Failed to stop stream checker: {e}")
        
        # Determine overall success
        all_stopped = (
            results['automation']['stopped'] and 
            results['stream_checker']['stopped']
        )
        ...
        logger.error(f"Error in stop_all_services: {e}")
        return jsonify({"error": str(e)}), 500
...
@app.route('/api/stream-checker/test-streams-with-incomplete-stats', methods=['POST'])
def test_streams_with_incomplete_stats():
    """Test all streams that have incomplete quality stats.
    
    This is useful for:
    - Retesting streams where FFmpeg only detected partial data (e.g., only bitrate)
    - Fixing streams where codec detection failed
    - Ensuring all streams have complete quality data
    
    Tests streams where stream_stats exists but is missing critical fields:
    - resolution
    - video_codec
    ...
    try:
        ...
        # Required fields for complete stats
        ...
        for channel in channels:
            ...
            for stream in streams:
                ...
                # Skip streams with no stats at all (handled by test-streams-without-stats)
                if not stream_stats or stream_stats == '{}' or stream_stats == 'null':
                    continue
                
                # Parse stream_stats if it's a string
...
@app.route('/api/stream-checker/test-m3u-account-streams/<int:account_id>', methods=['POST'])
def test_m3u_account_streams(account_id):
    """Test all streams from a specific M3U account that are assigned to channels.
    
    This is useful for:
    - Testing all streams from a specific provider
    - Verifying quality of a newly added M3U account
    - Retesting streams after provider changes
    
    Only tests streams that are currently assigned to channels.
    ...
    try:
        ...
        # Get UDI manager to find streams from this M3U account
        from udi.manager import get_udi_manager
        udi = get_udi_manager()
        ...
        for channel in channels:
            ...
            for stream in streams:
                ...
                stream_m3u_account = stream_data.get('m3u_account')
                if stream_m3u_account != account_id:
                    continue
                
                # Check if this stream should skip quality checking (quality exclusions)
                if service._should_skip_quality_check(stream_data):
                    # Skip quality-excluded streams
                    continue
        ...
        if not streams_to_test:
            return jsonify({
                "message": f"No streams from M3U account {account_id} found in channels",
                "streams_found": 0,
                "channels_affected": 0
            })
        ...
        return jsonify({
            "message": f"Queued {len(streams_to_test)} stream(s) from M3U account {account_id} for testing",
            "streams_found": len(streams_to_test),
