...
class AutomatedStreamManager:
    ...
    def discover_and_assign_streams(self, force: bool = False, skip_check_trigger: bool = False, enable_parallel_regex: bool = True, m3u_account_id: int = None) -> Dict[str, int]:
        ...
        try:
            ...
            # Only refresh streams if cache is empty (performance optimization)
            if not all_streams:
                logger.debug("Streams cache is empty, refreshing streams...")
                udi.refresh_streams()
                all_streams = get_streams(log_result=False)
                ...
                try:
                    ...
                    if channel_ids_to_mark:
                        try:
                            from stream_checker_service import get_stream_checker_service
                            stream_checker = get_stream_checker_service()
                            stream_checker.update_tracker.mark_channels_updated(channel_ids_to_mark, stream_counts=stream_counts, force_check=False, m3u_account_filter=m3u_account_id)
                            if m3u_account_id:
